...
def import_Excel_SUT_2014(year):
    ...
    df = pd.read_excel('India Supply Use Table SUT_12-13.xlsx', sheet_name='Supply 2012-13', header=None, skiprows=1)
    df_rates = pd.read_excel('India Supply Use Table SUT_12-13.xlsx', sheet_name='Rates')

    df1 = df.iloc[:supply_end_row-1,:supply_end_col]
    df1 = df1.fillna(0)
    df1.columns = df1.iloc[supply_row_sector_id-2,:]
    df1.index = df1.iloc[:,supply_col_product_id-2]
    df2 = df1.iloc[supply_start_row-2:,supply_start_col-1:]
    supply_mat = df1.values[2:,2:]
    rates = df_rates.values[:,1]
    rate_vec = rates.reshape(rates.shape[0], 1)
    tax_supply = supply_mat * rate_vec
    ...
    df_use = pd.read_excel('India Supply Use Table SUT_12-13.xlsx', sheet_name='Use 2012-13', header=None, skiprows=1)

    df1 = df_use.iloc[:use_end_row-1,:use_end_col]
    df1 = df1.fillna(0)
    df1.columns = df1.iloc[use_row_sector_id-2,:]
    df1.index = df1.iloc[:,use_col_product_id-2]
    df2 = df1.iloc[use_start_row-2:,use_start_col-1:]
    use_mat = df1.values[2:,2:]
    tax_use = use_mat * rate_vec
